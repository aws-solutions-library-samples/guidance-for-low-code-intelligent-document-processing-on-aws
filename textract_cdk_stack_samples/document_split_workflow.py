...
class DocumentSplitterWorkflow(Stack):
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        super().__init__(
            scope,
            construct_id,
            description="IDP CDK constructs sample for splitting large \
                documents, classifying and extracting informaiton based \
                    on the doc type (SO9217)",
            **kwargs,
        )
        ...
        # Remove the auto_delete_objects=True
        # and removal_policy=RemovalPolicy.DESTROY
        # when the documents should remain after
