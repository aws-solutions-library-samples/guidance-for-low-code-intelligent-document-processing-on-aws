...
class PaystubAndW2Spacy(Stack):
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        async_chain_with_config = sfn.Chain.start(textract_async_task_with_config).next(
        ...
        random_choice = (
            ...
            .when(
                sfn.Condition.number_greater_than("$.Random.randomNumber", 50),
                async_chain,
            )
        ...
        doc_type_choice = (
            ...
            .when(
                ...
                sfn.Pass(self, "DocumentTypeNotClear"),
            )
            .when(
                sfn.Condition.string_equals(
                    "$.classification.documentType", "AWS_OTHER"
                ),
                sfn.Pass(self, "SendToOpenSearch"),
            )
            .when(
                sfn.Condition.string_equals("$.classification.documentType", "AWS_ID"),
                textract_sync_task_id2,
            )
            .when(
                sfn.Condition.string_equals(
                    "$.classification.documentType", "AWS_EXPENSE"
                ),
                textract_sync_task_expense2,
            )
            .otherwise(configurator_task)
        )

        number_queries_choice = (
            sfn.Choice(self, "NumberQueriesChoice")
            .when(
                sfn.Condition.and_(
                    sfn.Condition.is_present("$.numberOfQueries"),
        ...
        number_pages_choice = (
            sfn.Choice(self, "NumberPagesChoice")
            .when(
                sfn.Condition.and_(
                    sfn.Condition.is_present("$.numberOfPages"),
        ...
        CfnOutput(
            ...
            value=spacy_classification_task.spacy_sync_lambda_log_group.log_group_name,
        )
        # CfnOutput(self,
        #           "DashboardLink",
        ...
        CfnOutput(self, "DBClusterARN", value=csv_to_aurora_task.db_cluster.cluster_arn)
        CfnOutput(
            self,
            "DBClusterSecretARN",
            value=typing.cast(
                rds.ServerlessCluster, csv_to_aurora_task.db_cluster
            ).secret.secret_arn,
        ...
        CfnOutput(
            ...
            value=typing.cast(rds.ServerlessCluster, csv_to_aurora_task.db_cluster)
            ._security_groups[0]
            .security_group_id,
        )  # pyright: ignore [reportOptionalMemberAccess]
