...
class AnalyzeIDStack(Stack):
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        super().__init__(
            scope,
            construct_id,
            description="IDP CDK constructs sample for Textract AnalyzeID (SO9217)",
            **kwargs,
        )
        ...
        workflow_chain_id = sfn.Chain.start(decider_task_id).next(textract_sync_task_id)

        state_machine = sfn.StateMachine(
            self,
            "AnalyzeID",
            definition_body=sfn.DefinitionBody.from_chainable(workflow_chain_id),
        )
