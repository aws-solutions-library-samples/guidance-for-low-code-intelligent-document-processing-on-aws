...
class AnalyzeExpenseStack(Stack):
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        super().__init__(
            scope,
            construct_id,
            description="IDP CDK constructs sample for Textract AnalyzeExpense (SO9217)",
            **kwargs,
        )
        ...
        workflow_chain_expense = sfn.Chain.start(decider_task_expense).next(
            textract_sync_task_expense
        )

        state_machine = sfn.StateMachine(
            self,
            "Expense",
            definition_body=sfn.DefinitionBody.from_chainable(workflow_chain_expense),
        )
